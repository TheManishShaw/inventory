<?php
    ...
    `total_amount`,`_tblproducts`.`code`,`_tblproducts`.`name`,`_tblproducts`.`cost`,
    `stock_tbl`.`stock` FROM `_tblpurchase_details` INNER JOIN `_tblproducts` ON 
    `_tblpurchase_details`.`product_id`=`_tblproducts`.`id` LEFT JOIN `stock_tbl` ON 
    `stock_tbl`.`product_id`=`_tblpurchase_details`.`product_id` AND `stock_tbl`.`store_id`='$u_set'
     WHERE `purchase_id`='$purchase_id' AND `_tblpurchase_details`.`status`='active'";
