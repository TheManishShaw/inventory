...
`_tblproducts`.`category_id`=`category_tbl`.`cat_id`) AS `quantity` FROM `category_tbl` WHERE 
...
`_tblproducts` WHERE `_tblproducts`.`brand_id`=`_brands`.`id` AND `status`='active') AS `quantity` FROM `_brands` 
