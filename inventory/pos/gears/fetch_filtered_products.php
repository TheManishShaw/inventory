<?php
    ...
    if(!empty($brand_id)){
        foreach($product_array as $item){
            $query1 = "SELECT * FROM `_tblproducts` WHERE `id` = '$item' AND `u_set` = '$store_id' 
        ...
        foreach($product_array as $item){
            $query1 = "SELECT *,(SELECT `name` FROM `category_tbl` WHERE `category_tbl`.`cat_id`=`_tblproducts`.`category_id`)
