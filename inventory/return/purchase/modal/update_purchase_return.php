<?php
    ...
    `net_tax`,`total_amount`,`return_reason`,`return_percent`,`absolute_tax`,`_tblproducts`.`code`,`_tblproducts`.`name`,
    `_tblproducts`.`cost`,`stock_tbl`.`stock` FROM `_tblpurchase_return_details` 
    INNER JOIN `_tblproducts` ON `_tblpurchase_return_details`.`product_id`=`_tblproducts`.`id`
    LEFT JOIN `stock_tbl` ON `stock_tbl`.`product_id`=`_tblpurchase_return_details`.`product_id` AND 
    `stock_tbl`.`store_id`='$u_set' WHERE `purchase_id`='$purchase_id' AND `_tblpurchase_return_details`.`status`='active'";
    $productsResult = mysqli_query($link,$query);
    if (!$productsResult){
        die('Could not fetch Purchase Return Details. '.mysqli_error($link));
    }

    $query = "SELECT `return_reason`,`return_percent` FROM `_tblreturn_reason` WHERE `return_type`='purchase' 
    AND `status`='active' AND `uset` = '$u_set'";
