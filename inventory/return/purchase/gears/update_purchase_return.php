<?php
    ...
    for($i = 0; $i < count($productIds); $i++) {
        ...
        `return_reason`,`return_percent`,`absolute_tax`,`u_set`,`status`,`created_at`) VALUES ('$purchase_id','$productIds[$i]','$product_tax[$i]','$product_subtotal[$i]',
        '$product_quantity[$i]','$return_reason[$i]','$return_percent[$i]','$absolute_tax[$i]','$u_set','active','$date');";
        $result = mysqli_query($link,$query);
        if (!$result) {
